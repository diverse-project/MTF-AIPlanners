...
def run_planner(timeout: int, eval: str, domain: str, problem: str, output: str):
    ...
    command = f'docker run --rm -v {path}:/host aibasel/downward --plan-file /host/{output} --overall-time-limit {timeout}s --overall-memory-limit 2G /host/{domain} /host/{problem} --evaluator "h={eval}()" --search "eager(single(sum([weight(g(), -1), weight(h, 10)])), reopen_closed=false, f_eval=sum([weight(g(), -1), weight(h, 10)]))"'
